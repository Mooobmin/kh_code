...
public class SessionTestServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (param.equals("create")) {
			...
			if(session.isNew()) { //세션이 새로이 만들어졌으면 true, 이미 만들어진 세션이면 false를 반환
				msg = "새로운 세션 객체가 생성됨";
			} else {
				msg = "기존의 세션 객체가 리턴됨";
			}
			...
			if(session != null) {
				session.invalidate(); //현재 생성된 세션을 무효화 시킴. 즉 현재 세션을 삭제한다.
				msg = "세션 객체 삭제 작업 완료";
			} else {
				msg = "삭제할 세션 존재하지 않음";
			}
			...
			session.setAttribute("msg", "메시지 입니다."); //세션 객체에 속성 추가(바인딩)
